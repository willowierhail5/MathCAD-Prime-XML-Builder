...
def create_matrix(var_name, matrix):
...
def append_variables(root, var_names, var_values, var_units):
...
def append_matrices(root, var_names, matrices):
    ...
    for name, matrix in zip(var_names, matrices):
        regions_tag.append(create_matrix(name, matrix))
...
def add_excels(root, var_names, file_names, ranges):
    ...
    for var_name, file_name, range in zip(var_names, file_names, ranges):
        regions_tag.append(create_read_excel(var_name, file_name, range))


def add_texts(root, texts):
    # not functioning
    regions_tag = root.find(r_s)
    for text in texts:
        regions_tag.append(create_text(text))


# Create a new FlowDocument as an XML element
def create_flowdocument():
    nsmap = {
    ...
    flow_document = ET.Element("FlowDocument", nsmap=nsmap, **attributes)
    flow_document.set("{http://www.w3.org/XML/1998/namespace}lang", "en-us")

    paragraph = ET.SubElement(flow_document, "Paragraph")
    paragraph.text = "This is a sample text for the FlowDocument."

    return flow_document


# Main code
flowdocument_xml = create_flowdocument()

# Write the content to a .xamlpackage file
with open("flowdocument.xamlpackage", "wb") as f:
    f.write(
        ET.tostring(
            flowdocument_xml, pretty_print=True, encoding="UTF-8", xml_declaration=True
        )
    )

print("FlowDocument saved to flowdocument.xamlpackage!")


def create_read_excel(var_name, file_name, range):
    region_id = state["region_id"]
    top = state["top"]
        ...
        with zipfile.ZipFile(
            ...
            for filename in myzip.namelist():
                if filename == "mathcad/result.xml":
                    ...
                    append_variables(root, var_names, var_values, var_units)
