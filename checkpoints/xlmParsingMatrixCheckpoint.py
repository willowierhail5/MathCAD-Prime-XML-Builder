...
def main():
    ...
    regions_tag = root.find("{http://schemas.mathsoft.com/worksheet50}regions")
    regions_tag.append(read_excel("b", ".\\copyRunningMathcad.xlsm", "Sheet1!B1:B100"))
    ...
    matrix_b = [[4, 8, 12, 16, 20, 24, 28, 32, 36, 40]]
    regions_tag = root.find("{http://schemas.mathsoft.com/worksheet50}regions")
    regions_tag.append(define_matrix("B", matrix_b))
    regions_tag.append(evaluate_var("B"))
    regions_tag.append(evaluate_var("NewVar1"))
    # Test the new create_operation function:
    regions_tag = root.find("{http://schemas.mathsoft.com/worksheet50}regions")
    regions_tag.append(create_operation("NewVar1", "NewVar2", "add"))
    regions_tag.append(create_operation("b", "NewVar2", "multiply"))
