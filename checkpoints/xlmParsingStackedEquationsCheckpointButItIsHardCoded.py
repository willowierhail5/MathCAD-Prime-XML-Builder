...
input_file_path = "TestStackedOperations.mcdx"
...
def create_operation(var_names, operations):
    ...
    op_map = {
        ...
        "multiply": "{" + ml_namespace + "}mult",
        "divide": "{" + ml_namespace + "}div",
    }
    ...
    def create_mult_operation_node(parent, var_name1, var_name2):
        outer_apply = create_operation_node(parent, "multiply")
        inner_apply = create_operation_node(
            outer_apply,
            "multiply",
            create_var_node(outer_apply, var_name2),
            create_var_node(outer_apply, var_name1),
        )
        create_var_node(outer_apply, var_name1)
        return outer_apply

    root_apply = ET.SubElement(eval, "{" + ml_namespace + "}apply")
    ET.SubElement(root_apply, op_map["add"])  # root operation is now always addition

    root_plus = create_operation_node(root_apply, "add")
    create_mult_operation_node(root_plus, var_names[0], var_names[1])
    create_mult_operation_node(root_plus, var_names[0], var_names[1])

    create_var_node(root_apply, var_names[1])
    ...
    with zipfile.ZipFile(input_file_path, "r") as myzip:
        for filename in myzip.namelist():
            if filename == "mathcad/result.xml":
                ...
                add_operations(root, var_names_list, operations_list)

                add_evals(
                    root, var_names + matrix_names + excel_var_names
                )  # add excel_var_names
                xml_string = ET.tostring(
                    root, pretty_print=True, xml_declaration=True, encoding="UTF-8"
                )
                data[filename] = xml_string
            else:
...
def main():
    ...
    write_data_to_zip(output_file_path, data)
